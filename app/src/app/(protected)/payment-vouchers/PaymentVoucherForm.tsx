...
export function PaymentVoucherForm({
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const result =
        mode === "create"
          ? await createPaymentVoucher(input)
          : await updatePaymentVoucher(voucherId!, input);
      if (result.error) {
        setError(result.error);
        return;
      }
      router.push(`/payment-vouchers/${result.id}`);
    });
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <section className="rounded-lg border border-slate-200 bg-white p-6">
        ...
        <div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-4">
          ...
          <label className="block">
            <span className="mb-1 block text-xs font-medium text-slate-600">仕入金額（参考）</span>
            <input
              type="number"
              step="0.01"
        ...
        <p className="mt-2 text-xs text-slate-400">
          請求金額・仕入金額・消費税額は、請求更新（締め処理）が未実装のため手入力で記録する参考項目です。
        </p>
      </section>

      <section className="rounded-lg border border-slate-200 bg-white p-6">
        <h2 className="mb-4 text-sm font-bold text-slate-600">支払明細</h2>
          ...
          <table className="w-full min-w-[760px] text-sm">
            <thead className="text-left text-xs text-slate-500">
              <tr>
                ...
                <th className="w-40 pb-2">支払元銀行</th>
      ...
      <div className="flex gap-3">
        ...
        <a href="/payment-vouchers" className="rounded px-6 py-2 text-sm text-slate-500 hover:bg-slate-100">
          キャンセル
        </a>
      </div>
