...
export default async function ReceiptVoucherDetailPage({
  ...
  const voucher = await prisma.receipt_vouchers.findUnique({
    where: { id: BigInt(id) },
    include: {
      receipt_voucher_lines: { orderBy: { line_no: "asc" }, include: { banks: true } },
      customers: true,
    },
  });
    ...
    <div className="max-w-4xl">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-lg font-bold text-slate-800">
          入金伝票 {voucher.voucher_no}
          {voucher.print_receipt && (
      ...
      <section className="mb-6 rounded-lg border border-slate-200 bg-white p-6">
        <dl className="grid grid-cols-2 gap-y-3 text-sm sm:grid-cols-4">
          ...
          <dt className="text-slate-500">売上金額（参考）</dt>
          <dd className="text-slate-800">
            {voucher.sales_amount != null ? Number(voucher.sales_amount).toLocaleString() : "-"}
          </dd>
      ...
      <section className="mb-6 rounded-lg border border-slate-200 bg-white p-6">
        <h2 className="mb-3 text-sm font-bold text-slate-600">入金明細</h2>
        <table className="w-full text-sm">
          <thead className="border-b border-slate-200 text-left text-xs text-slate-500">
            <tr>
              <th className="pb-2">区分</th>
              <th className="pb-2 text-right">金額</th>
              <th className="pb-2">振込先銀行</th>
              <th className="pb-2">手形決済日</th>
              <th className="pb-2">手形No</th>
              <th className="pb-2">備考</th>
            </tr>
          </thead>
          <tbody>
            {voucher.receipt_voucher_lines.map((l) => (
              <tr key={l.id} className="border-b border-slate-50">
                ...
                <td className="py-2 text-slate-500">{l.bill_no}</td>
