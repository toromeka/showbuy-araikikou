...
export default async function SalesVouchersPage({
  ...
  const where: Prisma.sales_vouchersWhereInput = {};
  if (q) {
    where.OR = [
  ...
  const [vouchers, total] = await Promise.all([
    prisma.sales_vouchers.findMany({
      where,
    ...
    prisma.sales_vouchers.count({ where }),
  ]);
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-lg font-bold text-slate-800">売上伝票（{total.toLocaleString()}件）</h1>
        <Link
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-100 text-left text-slate-600">
            <tr>
              ...
              <th className="px-4 py-2 text-right">消費税額</th>
            ...
            {vouchers.map((v) => (
              <tr key={v.id.toString()} className="border-t border-slate-100 hover:bg-slate-50">
                <td className="px-4 py-2 font-mono">
                  <Link href={`/sales-vouchers/${v.id}`} className="text-blue-600 hover:underline">
                    {v.voucher_no}
                  </Link>
                </td>
                ...
                <td className="px-4 py-2 text-right">{Number(v.sales_amount).toLocaleString()}</td>
                <td className="px-4 py-2 text-right">{Number(v.tax_amount).toLocaleString()}</td>
                <td className="px-4 py-2">
                  {v.is_cash_sale && (
                    <span className="mr-1 rounded bg-amber-100 px-2 py-0.5 text-xs text-amber-700">現金</span>
                  )}
                  {v.is_billed ? (
                    <span className="rounded bg-slate-100 px-2 py-0.5 text-xs text-slate-500">請求済</span>
                  ) : (
                    <span className="rounded bg-green-100 px-2 py-0.5 text-xs text-green-700">未請求</span>
                  )}
                </td>
              </tr>
            ))}
      ...
      {totalPages > 1 && (
        <div className="mt-4 flex gap-2 text-sm">
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
            <Link
              key={p}
              href={`/sales-vouchers?q=${encodeURIComponent(q)}&from=${from}&to=${to}&page=${p}`}
