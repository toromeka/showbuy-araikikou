...
export default async function PurchaseVouchersPage({
  ...
  const where: Prisma.purchase_vouchersWhereInput = {};
  if (q) {
    where.OR = [
  ...
  const [vouchers, total] = await Promise.all([
    prisma.purchase_vouchers.findMany({
      where,
    ...
    prisma.purchase_vouchers.count({ where }),
  ]);
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-lg font-bold text-slate-800">仕入伝票（{total.toLocaleString()}件）</h1>
        <Link
            ...
            {vouchers.map((v) => (
              <tr key={v.id.toString()} className="border-t border-slate-100 hover:bg-slate-50">
                <td className="px-4 py-2 font-mono">
                  <Link href={`/purchase-vouchers/${v.id}`} className="text-blue-600 hover:underline">
                    {v.voucher_no}
                  </Link>
                </td>
                ...
                <td className="px-4 py-2">
                  {v.is_settled ? (
                    <span className="rounded bg-slate-100 px-2 py-0.5 text-xs text-slate-500">支払済</span>
                  ) : (
                    <span className="rounded bg-green-100 px-2 py-0.5 text-xs text-green-700">未払</span>
                  )}
                </td>
              </tr>
            ))}
      ...
      {totalPages > 1 && (
        <div className="mt-4 flex gap-2 text-sm">
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
            <Link
              key={p}
              href={`/purchase-vouchers?q=${encodeURIComponent(q)}&from=${from}&to=${to}&page=${p}`}
